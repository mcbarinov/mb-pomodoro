...
def pause(ctx: typer.Context) -> None:
    ...
    if row is None or row.status != IntervalStatus.RUNNING or row.run_started_at is None:
        out.print_interval_error_and_exit("NOT_RUNNING", "No running interval to pause.", row)

    now = int(time.time())
    new_worked = row.effective_worked(now)

    if not db.pause_interval(conn, row.id, new_worked, now):
    ...
    out.print_paused(PauseResult(interval_id=row.id, worked_sec=new_worked, remaining_sec=remaining))
